...
# nflverse froze the legacy all-seasons player_stats.csv (stops at 2024).
# Current weekly stats live in per-season assets; column names shifted in
# the migration, so normalize back to the legacy names this script expects.
PER_SEASON_CANDIDATES = [
    f"{BASE}/stats_player/stats_player_week_{{season}}.csv",
    f"{BASE}/stats_player/stats_player_reg_{{season}}.csv",
    f"{BASE}/player_stats/player_stats_{{season}}.csv",
]
COL_ALIASES = {
    "team": "recent_team", "player_current_team": "recent_team",
...
def _i(v):
    """NaN-proof int: the new nflverse files carry NaN where the legacy
    file had 0, and `NaN or 0` evaluates to NaN (NaN is truthy)."""
    try:
        f = float(v)
...
def fetch_player_stats_season(season):
    """Try current nflverse per-season assets, newest naming first."""
    for tmpl in PER_SEASON_CANDIDATES:
        url = tmpl.format(season=season)
        try:
            df = fetch(url, url.rsplit("/", 1)[-1])
        except Exception as e:
            print(f"    not available ({e.__class__.__name__}), trying next…")
            continue
        ...
        if "season_type" in df.columns:
            df = df[df.season_type.isin(["REG", "REG+POST", "regular"]) |
                    df.season_type.isna()]
        return df
    return None
...
def seed_from_nflverse(seasons=SEASONS):
    ...
    try:
        ...
        print(f"  Loaded {len(stats):,} weekly records (legacy all-seasons file)")
    except Exception as e:
        print(f"  Legacy file failed ({e}); will rely on per-season assets.")
        stats = pd.DataFrame()
    # legacy file is frozen at 2024 — top up missing seasons per-season
    have = set(stats['season'].unique()) if len(stats) else set()
    for season in seasons:
        if season in have:
            continue
        extra = fetch_player_stats_season(season)
        if extra is None:
            print(f"  WARNING: no per-season asset found for {season}")
            continue
        keep = [c for c in extra.columns if c in stats.columns] or None
        stats = pd.concat([stats, extra[keep] if keep else extra],
                          ignore_index=True)
        print(f"  Added {season}: {len(extra):,} weekly records")
    if stats.empty:
        print("  ERROR: no data from any source — check connection")
        return
            ...
            for _, log in player_logs.iterrows():
                ...
                if pos == 'QB':
                    ...
                    """, (pid, season, week, team, opp,
                          ...
                          _i(log.get('passing_yards')),
                          _i(log.get('attempts')),
                          _i(log.get('completions')),
                          _i(log.get('passing_tds')),
                          _i(log.get('interceptions')),
                          _i(log.get('rushing_yards'))))
                    ...
                    """, (pid, season, week, team, opp,
                          ...
                          _i(log.get('rushing_yards')),
                          _i(log.get('carries')),
                          _i(log.get('receptions')),
                          _i(log.get('receiving_yards')),
                          _i(log.get('targets'))))
