...
def get_batter_season_stats(player_id):
    ...
    try:
        ...
        s = splits[0].get("stat", {})
        return {
            "avg":    float(s.get("avg",   ".000").lstrip(".")),
            "hits":   int(s.get("hits",   0)),
            "ab":     int(s.get("atBats", 0)),
            "games":  int(s.get("gamesPlayed", 0)),
            "ops":    float(s.get("ops",  ".000").lstrip(".")),
            "slg":    float(s.get("slg",  ".000").lstrip(".")),
            "obp":    float(s.get("obp",  ".000").lstrip(".")),
...
def get_batter_last_n_days(player_id, days=14):
    ...
    try:
        ...
        return {
            ...
            "bb":      int(s.get("baseOnBalls", 0)),
            "so":      int(s.get("strikeOuts", 0)),
            "slump":   int(s.get("hits", 0)) / max(int(s.get("atBats", 1)), 1) < 0.150,
        }
